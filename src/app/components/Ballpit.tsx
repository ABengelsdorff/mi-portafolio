...
class X {
 ...
 get postprocessing():
  | { setSize: (w: number, h: number) => void; render: () => void; dispose: () => void }
  | undefined {
      ...
      if ((obj as Mesh).isMesh) {
        ...
        if (Array.isArray(material)) {
          material.forEach((mat) => {
            if (mat instanceof Material) mat.dispose();
          });
   ...
   class Y extends MeshPhysicalMaterial {
    ...
    constructor(params: Partial<MeshPhysicalMaterialParameters>) {
      ...
      this.onBeforeCompile = (shader) => {
        ...
        // Llamado seguro al hook extra si existe
...
function createBallpit(
  canvas: HTMLCanvasElement,
  config: Partial<typeof XConfig & WConfig> = {}
): CreateBallpitReturn {
  ...
  function initialize(cfg: Partial<typeof XConfig & WConfig>) {
    if (spheres) {
      threeInstance.clear();
